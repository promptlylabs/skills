...
1. Create/update `plugins/promptly-skills/skills/<name>/SKILL.md`.
2. Add/update canonical skill in `README.md` Available Skills table (alphabetical; exclude alias/symlink entries).
3. Add/update `Skill(promptly-skills:<name>)` in `.claude/settings.json`.
4. Add/update skill allowlist in `plugins/promptly-skills/skills/claude-settings-audit/SKILL.md`.
...
```bash
uv run plugins/promptly-skills/skills/skill-writer/scripts/quick_validate.py <path/to/skill-directory> --strict-depth
```
